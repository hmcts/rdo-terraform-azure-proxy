...
resource "null_resource" "ansible-runs" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "cd ~/ansible",
      "sudo ansible-playbook ~/ansible/proxy.yml -i ~/ansible/hosts -e arm_tenant_id=${var.arm_tenant_id} -e arm_client_id=${var.arm_client_id} -e arm_client_secret=${var.arm_client_secret} -e palo_subnet=${var.palo_subnet} -e azcontainer-password=${var.azcontainer-password}",
    ]
